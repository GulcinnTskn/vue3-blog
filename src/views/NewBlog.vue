...
export default {
  data(){
    return {
      categoryList : [],
      userData : { //user etkileşimi yapılan data
        title: null,
        url: null,
        categoryId: null,
        description: null
  ...
  mounted(){ //lifecycle hookudur. sayfa yüklendiğinde bilgiyi çekeceğimiz için
  ...
  methods: {
    onSave(){
      ...
      this.$appAxios.post("/blogs", saveData).then(save_blog_response => {
        console.log(save_blog_response);

        Object.keys(this.userData)?.forEach(field => (this.userData[field] = null));
          const socketData = {
          ...save_blog_response.data,
          user: this._getCurrentUser,
          category: this.categoryList?.find(c => c.id === saveData.categoryId)
        };
        this.$socket.emit("NEW_BLOG_EVENT", socketData);
        // this.$router.push({name:'HomePage'}) // kullanıcı her kaydetmeden sonra ana sayfaya gitmesin diye kapattım
      })
    }
  },
  computed : {
