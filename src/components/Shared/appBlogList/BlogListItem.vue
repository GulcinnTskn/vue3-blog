<template>
            ...
            <div class="p-3">
              ...
              <div class="flex items-center justify-center mt-2 gap-x-1">
                ...
                <button @click="blogItem" class="bookmark-btn group bookmark-item-active" :class="{'bookmark-item-active':alreadyBloged}">
...
export default {
  ...
  computed : {
    ...
    ...mapGetters(["_getCurrentUser", "_userLikes", "_userBlogs","_userBlogs"]),
    alreadyLiked(){
      return this._userLikes?.indexOf(this.item.id) > -1
    },
    alreadyBloged(){
      return this._userBlogs?.indexOf(this.item.id) > -1
    }
  },
  methods : {
    likeItem(){
    ...
    blogItem(){
      console.log("_userLikes", this._userBlogs);

       let blogs = [...this._userBlogs]
      if (!this.alreadyBloged) {
        blogs = [...blogs, this.item.id]
      }else{
        blogs = blogs.filter(b => b !=this.item.id)
      }

      this.$appAxios.patch(`/users/${this._getCurrentUser.id}`, {blogs}).then(blog_response =>{
